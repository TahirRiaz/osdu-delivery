...
/// <summary>
/// Object enumeration, pagination, filtering, and search for the MySQL catalog reader. The reader hides the
/// server-maintained databases (<c>mysql</c>, <c>information_schema</c>, <c>performance_schema</c>, <c>sys</c>),
/// so these run against a dedicated <c>sf_itest</c> database provisioned once by <see cref="MySqlListingFixture"/>
/// (which needs CREATE DATABASE; if the login lacks it, or MySQL is unreachable, every test here skips rather
/// than fails). Gated on <c>SQLFLOW_TEST_MYSQL</c>.
/// </summary>
[Trait("Category", "Integration")]
public sealed class MySqlListingTests : IClassFixture<MySqlListingFixture>
{
    private const DataSourceKind Kind = DataSourceKind.MySQL;
    private readonly MySqlListingFixture _fixture;

    public MySqlListingTests(MySqlListingFixture fixture) => _fixture = fixture;
    ...
    public async Task ListSchemas_IncludesTheTestSchema_ExcludesSystemByDefault()
    {
        ...
        Assert.Contains(schemas, s => string.Equals(s.Name, MySqlListingFixture.Schema, StringComparison.OrdinalIgnoreCase));
        Assert.DoesNotContain(schemas, s => string.Equals(s.Name, "mysql", StringComparison.OrdinalIgnoreCase));
        ...
        Assert.Contains(withSystem, s => string.Equals(s.Name, "information_schema", StringComparison.OrdinalIgnoreCase));
    }
    ...
    public async Task SearchObjects_FindsByTerm()
    {
        RequireFixture();
        await using var connection = await ForeignDb.OpenAsync(Kind, _fixture.ConnectionString);
        var matches = await ForeignDb.Reader(Kind).SearchObjectsAsync(connection, MySqlListingFixture.Schema, "sf_l_");
    ...
    private static ObjectScope Scope() => new() { Schema = MySqlListingFixture.Schema };
}
