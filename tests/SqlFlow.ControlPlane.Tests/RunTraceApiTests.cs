    ...
    public async Task Trace_MergesStatementsAndEvents_PagesInSql_ResolvesPipelineLatest_AndRendersText()
    {
        ...
        try
        {
            await using (var db = CatalogDatabase.Create(cs))
            {
                ...
                db.Runs.Add(SeedRun(oldRunId, pipelineId, repoId, flowName, t0.AddHours(-1)));
                ...
                db.Runs.Add(SeedRun(runId, pipelineId, repoId, flowName, t0.AddSeconds(30)));
            ...
            // The runs list carries each run's newest trace event as its "last action" (the batch overview's
            // live column): the newest event for the new run, the older run's own event for the older run.
            var listed = await GetJsonAsync<PagedResult<RunSummaryDto>>(
                client, token, $"/api/v1/runs?pipelineId={pipelineId}&pageSize=50");
            var newest = listed.Items.Single(r => r.RunId == runId);
            Assert.Equal("read 'a.csv' (31 row(s))", newest.LastAction);
            Assert.NotNull(newest.LastActionUtc);
            var older = listed.Items.Single(r => r.RunId == oldRunId);
