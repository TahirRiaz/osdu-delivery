...
internal static class Program
{
    ...
    private static SourceSpec SourceFromPath(string path, string[] args, bool withFlatten = false, bool suppressProvenance = false)
    {
        ...
        var treatAsFolder = Directory.Exists(path)
            || (isUri && (GetOption(args, "--pattern") is not null || !UriPathLooksLikeFile(path)));
    ...
    private static string UriLastSegment(string path)
    {
    ...
    private static bool UriPathLooksLikeFile(string path)
    {
        var ext = Path.GetExtension(UriLastSegment(path)).TrimStart('.').ToLowerInvariant();
