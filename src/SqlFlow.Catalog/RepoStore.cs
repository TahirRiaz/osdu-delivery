...
public sealed record RepoDeletionResult(
    int Pipelines, int Runs, int RunGroups, int Schedules, bool SourceRemoved);
...
/// than a cascade the database performs. Global rows a repo only contributes to (catalog object and
...
public static class RepoStore
{
        ...
        CatalogDbContext catalog, Guid repoId, CancellationToken ct = default)
    {
        ...
        return CatalogTransaction.InSerializableAsync<RepoDeletionResult?>(catalog, async () =>
        {
            ...
            return new RepoDeletionResult(pipelines, runs, runGroups, schedules, sourceRemoved);
        }, ct);
    }
}
