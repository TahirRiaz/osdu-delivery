...
/// resolver composition as full mode runs behind an in-memory store; the destination is the local filesystem
/// default and the run log stays the no-op default. With no declared invokes the postInvoke hook stays
/// unavailable (a set alias is a clear error, the documented without-database default). The source is SQL
/// Server by the document loader's guarantee, so the built-in SQL Server provider set is all the resolver
/// needs. Both the CLI and the tests construct through here, so YAML execution has exactly one code path.
/// </summary>
