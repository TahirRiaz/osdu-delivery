...
# 3. Deploy. The control plane runs ONE replica (the run queue lives in it and dispatch has one owner;
#    a second replica refuses every node call that lands on it), so its update always carries the pin.
#    deploy-prod.ps1 does this on every control-plane deploy; by hand, add it to the control-plane update:
