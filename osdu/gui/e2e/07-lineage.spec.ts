...
test.describe.serial("lineage", () => {
  ...
  test("an OSDU type opens with the pipeline that writes it, and jumps to the graph", async ({ adminPage }) => {
    ...
    await expect(provenance.getByText("recall-welllog", { exact: true })).toBeVisible({ timeout: 30_000 });
    ...
    await expect(graph.getByText("recall-welllog", { exact: true }).first()).toBeVisible();
  });
