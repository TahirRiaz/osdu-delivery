...
/// <summary>
/// Where a record's root row was read: the scope file (its full path or URI) and the row's 1-based position among the
/// data rows of that file. It is what lets a stored record, and the delivery built from it, be traced to the exact line
/// of the exact file the preparing side wrote.
/// </summary>
public readonly record struct SourceOrigin(string File, long Row);
...
public sealed record SourceRecord
{
    ...
    /// <summary>The file and row the root row was read from, when it was read from a drop.</summary>
