...
/// <summary>How long a submission whose files have not landed waits before the resume service finishes it (section <c>Osdu:Submissions</c>).</summary>
public sealed class SubmissionResumeOptions
{
    /// <summary>The configuration section the control plane module binds this from.</summary>
    public const string SectionName = "Osdu:Submissions";

    /// <summary>Turns the resume sweep off. An accepted submission whose request died then waits for a repeat of the request.</summary>
    public bool Enabled { get; set; } = true;

    /// <summary>Seconds between sweeps.</summary>
    public int PollSeconds { get; set; } = 30;

    /// <summary>Seconds an accepted or landed submission is left to its own request before the sweep takes it over.</summary>
    public int GraceSeconds { get; set; } = 60;

    /// <summary>Submissions one sweep finishes.</summary>
    ...
    public void Validate()
    {
        ...
        if (GraceSeconds < 1)
        {
            throw new InvalidOperationException($"{SectionName}:GraceSeconds must be positive.");
        }

        if (BatchSize is < 1 or > 500)
        {
            throw new InvalidOperationException($"{SectionName}:BatchSize must be between 1 and 500.");
        }
    }
}
