...
internal static class DeliveryVerbs
{
    ...
    public static async Task<int> CheckAsync(IServiceProvider provider, string flowPath, string[] args, bool json, CancellationToken ct)
    {
        ...
        // A flow reading from SQL has no drop until a run extracts one, so only a drop named explicitly is checked.
        var extractsFromSql = runtime.Flow.Source.Sql is not null && dropOverride is null;
        if (extractsFromSql)
        {
            result["source"] = "sql";
        }
        else if (dropOverride is not null || Directory.Exists(runtime.DropLocation) || runtime.DropLocation.Contains("://", StringComparison.Ordinal))
        {
            try
            {
        ...
        Console.WriteLine(dropChecked
            ...
            : extractsFromSql
                ? $"    drop        extracted from source.sql by each run, under {runtime.DropLocation}"
