...
public sealed class IdModifierTests
{
    ...
    private static string Entry(string target, string modifiers, string source = "dataset.unit", string extra = "") => $"""
          - target: {target}
            source: {source}
            modifiers:
        {modifiers}
    ...
    public void An_id_template_is_read_into_its_tokens_and_the_columns_cache_types_and_parameters_it_reads()
    {
        var mapping = TestSchema.Mapping(
            Entry("osdu.data.Unit", $"      - trim\n      - id: {UnitId}")
            + Entry("osdu.data.WellboreID", "      - id: \"{param.dataPartition}:master-data--Wellbore:{dataset.source}-{value}:{dataset.version}\"", "dataset.uwi")
            + Entry("osdu.data.Symbol", "      - id: \"{param.dataPartition}:reference-data--LogCurveFamily:{cache.CurveClasses.curve_family}:\"", "dataset.mnemonic"));
    ...
    [InlineData("      - id: \"dev:reference-data--X:{nope}:\"", "{nope} is not a token; an id template reads {value}")]
    [InlineData("      - id: \"dev:reference-data--X:{dataset.a.b.c}:\"", "reads the row as {dataset.<column>}")]
    ...
    [InlineData("      - id: \"dev:reference-data--X:{value} x:\"", "'U+0020' at position 30 is not a character an OSDU id carries")]
    [InlineData("      - id: \"dev:reference-data--X:{value}/x:\"", "'/' at position 30 is not a character an OSDU id carries")]
    [InlineData("      - id: \"dev:reference-data--X:{value}%zz:\"", "the '%' at position 30 starts no percent-escape")]
    ...
    [InlineData("      - id: \"{param.dataPartition}:reference-data--X:{dataset.curves.unit}:\"", "only an entry inside a repeater (a target with []) reads child rows")]
    [InlineData("      - id: " + UnitId + "\n      - id: " + UnitId, "an entry builds one id, and its modifiers list id 2 times")]
    [InlineData("      - id: " + UnitId + "\n      - trim", "id builds what the entry writes, so it is the last modifier; move trim before it")]
    public void A_template_that_could_never_give_an_id_is_refused_when_the_mapping_is_read(string modifiers, string expected)
    {
        var ex = Assert.Throws<FlowValidationException>(() => TestSchema.Mapping(Entry("osdu.data.Unit", modifiers)));
        ...
        var ex = Assert.Throws<FlowValidationException>(() => TestSchema.Mapping($"""
              - target: osdu.data.Unit
                ...
                modifiers:
                  - id: {UnitId}
            """));
        Assert.Contains("cache.UnitOfMeasure.id already gives what it writes", ex.Message, StringComparison.Ordinal);
    }
    ...
    {
        var result = Renderer(Entry("osdu.data.Unit", $"      - id: {UnitId}")).Render(Record(("unit", value)));
    ...
    {
        var result = Renderer(Entry("osdu.data.Unit", $"      - id: {UnitId}", extra: "    required: false")).Render(Record(("unit", "a\uD800b")));
    ...
    public void Row_columns_parameters_and_a_version_build_a_master_data_reference_the_schema_pattern_takes()
    {
        var renderer = Renderer(Entry(
            "osdu.data.WellboreID",
            "      - id: \"{param.dataPartition}:master-data--Wellbore:{dataset.source}-{value}:{dataset.version}\"",
            "dataset.uwi"));
        ...
        Assert.Contains(
            "osdu.data.WellboreID: the id {param.dataPartition}:master-data--Wellbore:{dataset.source}-{value}:{dataset.version} cannot be built, since {dataset.version} has no value, and the entry is required",
    ...
    {
        const string Modifier = "      - id: \"{param.dataPartition}:reference-data--UnitOfMeasure:{dataset.code}:\"";
        var required = Renderer(Entry("osdu.data.Unit", Modifier));
        var noCode = required.Render(Record(("unit", "m"), ("code", null)));
        Assert.Contains("since {dataset.code} has no value, and the entry is required", Assert.Single(noCode.Holds), StringComparison.Ordinal);
        ...
        var optional = Renderer(Entry("osdu.data.Unit", Modifier, extra: "    required: false")).Render(Record(("unit", "m"), ("code", "")));
    ...
    {
        var renderer = Renderer(Entry("osdu.data.Unit", $"      - id: {UnitId}"));
    ...
    public void An_id_the_variable_does_not_take_holds_the_record_whatever_required_says()
    {
        ...
        var unpinned = Renderer(Entry("osdu.data.WellboreID", "      - id: \"{param.dataPartition}:master-data--Wellbore:{value}\"", "dataset.uwi", "    required: false"))
        ...
        var named = Renderer(Entry("osdu.data.Unit", "      - id: \"{param.dataPartition}:{dataset.group}--UnitOfMeasure:{value}:\"", extra: "    required: false"));
    ...
    public void A_cache_token_reads_the_row_keyed_by_the_value_and_records_every_row_and_unlisted_key()
    {
        ...
        var required = Renderer(Entry("osdu.data.Symbol", Modifier));
        ...
        var zz = Renderer(Entry("osdu.data.Symbol", Modifier, extra: "    required: false")).Render(Record(("unit", "zz")));
    ...
    public void A_cache_token_holds_what_the_cache_cannot_answer_for_certain()
    {
        ...
        {
            var result = Renderer(Entry("osdu.data.Symbol", modifier, extra: "    required: false"), cache).Render(Record(("unit", unit)));
    ...
    public void The_gate_checks_what_an_id_reads_and_the_ids_it_gives_before_any_row_arrives()
    {
        ...
        Assert.DoesNotContain(Check(Entry("osdu.data.Unit", $"      - id: {UnitId}")), i => i.Target == "osdu.data.Unit");
        Assert.DoesNotContain(Check(Entry("osdu.data.WellboreID", "      - id: \"{param.dataPartition}:master-data--Wellbore:{value}:\"", "dataset.uwi")), i => i.Target == "osdu.data.WellboreID");
        ...
        Assert.Contains(
            ...
            Error("osdu.data.Unit", "      - id: \"{param.dataPartition}:master-data--Wellbore:{value}:\""),
            StringComparison.Ordinal);
        Assert.Contains(
            ...
            Error("osdu.data.WellboreID", "      - id: \"{param.dataPartition}:master-data--Wellbore:{value}\""),
            StringComparison.Ordinal);
        Assert.Contains("the id modifier gives an OSDU id, written as text, but the template takes a number at osdu.data.Weight.", Error("osdu.data.Weight", $"      - id: {UnitId}"), StringComparison.Ordinal);
        Assert.Contains("but the template takes a date-time string at osdu.data.When.", Error("osdu.data.When", $"      - id: {UnitId}"), StringComparison.Ordinal);
        Assert.Contains("osdu.data.Symbol) writes an OSDU id, but the template does not mark osdu.data.Symbol as a relationship.", Warning("osdu.data.Symbol", $"      - id: {UnitId}"), StringComparison.Ordinal);
        Assert.Contains(
            "a token writes the entity type of the id {param.dataPartition}:{dataset.group}--Wellbore:{value}:, so whether each id is one osdu.data.WellboreID takes is checked record by record",
            Warning("osdu.data.WellboreID", "      - id: \"{param.dataPartition}:{dataset.group}--Wellbore:{value}:\""),
            StringComparison.Ordinal);

        const string Family = "      - id: \"{param.dataPartition}:reference-data--LogCurveFamily:{cache.CurveClasses.";
        Assert.Contains("the id reads {cache.CurveClasses.curve_family}, and cache version 'refs-1' holds no CurveClasses. Cached: UnitOfMeasure, Wellbore.", Error("osdu.data.Symbol", Family + "curve_family}:\"", Cache()), StringComparison.Ordinal);
        Assert.Contains("and no CurveClasses row holds 'colour' in cache version 'refs-1'. Cached: curve_family, mnemonic, unit.", Error("osdu.data.Symbol", Family + "colour}:\""), StringComparison.Ordinal);
        Assert.Contains("the key CurveClasses is looked up by, which is the value itself; write {value}.", Check(Entry("osdu.data.Symbol", Family + "mnemonic}:\"")).Single(i => i.Message.Contains("itself", StringComparison.Ordinal)).Message, StringComparison.Ordinal);
        Assert.Contains(
            "UnitOfMeasure holds OSDU records (reference-data--UnitOfMeasure), which have no key to look the value up by",
            Error("osdu.data.Symbol", "      - id: \"{param.dataPartition}:reference-data--X:{cache.UnitOfMeasure.Code}:\""),
            StringComparison.Ordinal);
        var empty = Check(Entry("osdu.data.Symbol", "      - id: \"{param.dataPartition}:reference-data--X:{cache.Empty.value}:\""), Cache(new ReferenceType("Empty", ReferenceType.LookupEntityType("Empty"), [], "key")));
        ...
        var optionalParameter = new DeliveryDocumentLoader().ParseMapping(
            TestSchema.MappingDocument(Entry("osdu.data.Unit", "      - id: \"{param.dataPartition}:reference-data--UnitOfMeasure:{param.system}-{value}:\""))
        ...
        var missingColumn = Check(Entry("osdu.data.WellboreID", "      - id: \"{param.dataPartition}:master-data--Wellbore:{value}:{dataset.version}\"", "dataset.uwi"), columns: columns);
    ...
    public void An_id_modifier_is_written_back_by_the_builder_and_its_mistakes_are_named_by_target()
    {
        ...
        var original = loader.ParseMapping(TestSchema.MappingDocument(
            Entry("osdu.data.Unit", $"      - trim\n      - id: {UnitId}")
            + Entry("osdu.data.Symbol", "      - id: \"{param.dataPartition}:reference-data--LogCurveFamily:{cache.CurveClasses.curve_family}:\"")), "m.yaml");
        ...
        Assert.Contains(
            ...
            i => i.Target == "osdu.data.Unit" && i.Message.Contains("choose a dataset column as the input", StringComparison.Ordinal));
    }
}
