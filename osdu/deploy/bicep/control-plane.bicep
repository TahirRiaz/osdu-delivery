// Deploys the OSDU Delivery control plane as an always-on Azure Container App: the warm API the GUI, the CLI and
// any external scheduler call. It runs with a user-assigned managed identity that reads its secrets (the catalog
// connection and the JWT signing key) from Key Vault, and that SAME identity is what the engine uses to resolve
// ${keyvault:...} references at run time (SQLFLOW_AZURE_AUTH=mi), so no secret value is ever placed in this
// template or in app configuration. Provide an existing Container Apps environment and Key Vault; everything else
// is created here.
//
//   az deployment group create -g <rg> -f control-plane.bicep \
//     -p managedEnvironmentId=<env-id> image=<registry>/osdu-delivery-control-plane:latest keyVaultName=<kv>
//
// The OSDU module's `osdu` schema lives in the catalog database by default, so this tier needs no second
...
param name string = 'osdu-delivery-control-plane'
...
// Proxy trust is all-or-nothing: enabling it while trusting no networks fails startup validation, so the enable
// switch rides on the network list being non-empty.
