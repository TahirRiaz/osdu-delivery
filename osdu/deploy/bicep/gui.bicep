// Deploys the OSDU Delivery GUI as a Container App: the built SPA served by unprivileged nginx. The API base URL
// is injected at container start (the image writes /config.json from SQLFLOW_API_BASE_URL), so one image serves
...
//     -p managedEnvironmentId=<env-id> image=<registry>/osdu-delivery-gui:latest apiBaseUrl=<control-plane-url>

@description('Azure region. Defaults to the resource group location.')
param location string = resourceGroup().location

@description('Name of the Container App (and the prefix for its managed identity).')
param name string = 'osdu-delivery-gui'
