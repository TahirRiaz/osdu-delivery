// Provisions the Microsoft Entra app users sign in to OSDU Delivery with, so the estate's single sign-on is
// managed as code instead of by hand. It creates three directory objects through the Microsoft Graph Bicep
// extension:
//
...
//
// The role value stays SqlFlow.User: the scope names the platform issues are kept, as the project's naming rule
// says, and changing it on a live registration would revoke every existing assignment.
//
...
param uniqueName string = 'osdu-delivery'
