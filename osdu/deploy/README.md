...
- **The OSDU ledger on a node.** The delivery engine reads and writes the `osdu` schema per record while it plans
  and delivers. On the control plane that schema rides on the catalog connection; on a node, which has none, the
  module database needs a connection reference of its own. See
...
GUI at <http://localhost:8081>, API at <http://localhost:5000>. Bootstrap provisioning creates the catalog
database (the compose file turns on `ControlPlane__Bootstrap__AllowCreate`; the default only migrates a database
that already exists), applies SQLFlow's catalog migrations and then the OSDU module's, seeds roles, and creates the
admin from `.env` on first start.
...
| Template | Deploys |
| --- | --- |
| `main.bicep` | The full estate: Log Analytics and the Container Apps environment, a Key Vault holding every secret, the Azure SQL catalog plus the pre and ingestion databases, and the three apps below. |
...
- **Secrets live in Key Vault, read by managed identity**: no secret value appears in the templates or app
  ...
  (`SQLFLOW_AZURE_AUTH=mi`). The pre and ingestion connections are wired for free under their fixed names. The
  OSDU credentials a pool's flows use, and the OSDU module database connection the nodes need, are added via
...
- **Secrets stay on the tier that uses them**: the control plane gets the catalog connection and JWT material;
  nodes get a node token, the git token, the OSDU module database connection, the pre and ingestion connections
  and every `${env:...}` reference their pool's flows use, and never the catalog connection. Nothing data-plane
