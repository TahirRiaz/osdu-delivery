...
| --- | --- | --- |
| `core/<service>` | Storage, File, Dataset, Workflow, Search, Legal, Schema, Register, Entitlements | OpenAPI, copied from the core specification set in `D:\Projects\eq\src\osdu-csharp-client-main\openapi_specs` so the route tests run without it |
...
specification set when it changes. Review the diff: a changed contract is a change to the route built on it, and
