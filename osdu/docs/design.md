...
| read by id | a record page's In OSDU tab, following a record the document refers to; `POST /api/v1/delivery/flows/{pipelineId}/osdu/read` | Reads any OSDU record by its id through the flow's route and credentials, on a node, at its latest version. A read changes nothing, so it is a compute task with who asked rather than an activity. |
