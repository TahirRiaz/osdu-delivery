...
| `plan` | CLI: `sqlflow run <flow.yaml> --operation plan`; GUI and API: a run with operation `plan` | Renders documents and reports what would be created, updated, skipped or held. Works against the pinned template and the cache version read from the catalog, and without OSDU unless the mapping searches: a search asks the platform's search service for the records it refers to, and nothing else ([0009](decisions/0009-searched-references.md)). Changes nothing. |
| `preview` | CLI: `sqlflow preview <flow.yaml> [--key <key>]`; GUI: a delivery flow's Preview tab and a record page's Compare tab; API: `POST /api/v1/delivery/flows/{pipelineId}/preview`, `POST /api/v1/delivery/records/{flowId}/{key}/preview` | One record through the same planner a run plans with, twice over its one row: against the ledger, for what the next run would do with it, and without the ledger, which renders the document as a first delivery would even for a record a run would skip. Adds the document as the route sends it (a placeholder where the platform mints a value), the route's requests in order, the payload files with each parquet file's shape, and what the document refers to. Not a run: a node task in the GUI, since only a node holds the flow's connection. Changes nothing. |
...
| read by id | a record page's In OSDU tab, following a record the document refers to; `POST /api/v1/delivery/flows/{pipelineId}/osdu/read` | Reads any OSDU record by its id through the flow's route and credentials, on a node, at its latest version or at the version the request names (`version`), with the versions the target keeps of it (storage's `GET /records/versions/{id}`, newest first; null for a target that keeps no list, such as a DDMS). A read changes nothing, so it is a compute task with who asked rather than an activity. |
