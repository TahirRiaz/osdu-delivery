...
| [drop-contract.md](drop-contract.md) | What the preparing side writes: the manifest (as a JSON document, or as rows in CSV, JSON Lines or Parquet), the scope files in Parquet, CSV or JSON with their parsing options, the payload chunks, the delivery key and the payload hash it must derive. |
| [sql-source.md](sql-source.md) | The flow that reads its records from SQL Server or Azure SQL: the `source.sql` block, how an extraction runs inside one transaction and becomes a drop, the incremental watermark, and what a change must move to be delivered again. |
| [replica.md](replica.md) | A flow's replica: every drop's metadata rows loaded into SQL Server or Azure SQL, typed and evolved with SQLFlow's type inference and schema evolution, and planned from, so a re-run never reads its drop, a fan-out spreads slices of the loaded records, and a replan carries a mapping or cache change to every record without anyone sending a drop again. |
...
| `src/SqlFlow.Delivery/Replica` | The replica: SQLFlow's ported type inference, profiler, locale conversion, column transforms, schema evolution, DDL applier and keyed upsert (`Replica/Schema`), and the store that loads, reads, replans and prunes a flow's replica. |
