...
| `GET /flows/{pipelineId}/target` | read | Where the flow's records live: endpoint as declared, data partition, protocol, auth type, and the path each removal scope calls. On the ddms route the paths are those of the collection serving the kind the flow's synced mapping renders, and `ddms` says which collection of which DDMS that is (null on the other routes); a scope the flow cannot route reads `(not routable: ...)` and is not offered. |
...
| `POST /records/{flowId}/{key}/release`, `/redeliver`, `/verify` | operate | Release one record; redeliver it (`scope`: `all`, `record`, or `files` or `bulk` as the record's route sends them, a part the route does not send being refused with 400; `metadata` and `payload` name the same parts; `run` true queues a deliver run scoped to the record, which reads it from the ingestion tables by key under its last submission's parameter values, marks it with that scope and sends it); queue a verify run scoped to it. |
...
1,000), `redeliver` (what a run scoped to `recordKeys` sends again: `all`, the default, `record`, or `files` on the file and manifest routes and `bulk` on the ddms route; `metadata` and `payload` name the same parts, and a part the route does not send fails the run),
