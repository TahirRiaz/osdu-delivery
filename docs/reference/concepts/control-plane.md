...
| `GET /delivery/...` | The delivery ledger reads: a flow's stats, records, and submissions; one record with its attempts and activities; one submission with its attempts; the audit trail; the mappings and snapshots the repositories hold; the saved templates and one laid out variable by variable (`GET /delivery/templates`, `/templates/detail`, `/templates/schema`, `POST /delivery/templates/preview`); and the mapping builder's repositories, drafts, YAML and checks (`GET /delivery/mapping-builder/repos`, `POST /delivery/mapping-builder/draft`, `/compose`, `/parse`; src/SqlFlow.ControlPlane/Api/DeliveryTemplateEndpoints.cs). Documented in [Delivery operations](../../delivery/operations.md) |
...
| `POST /delivery/templates/search`, `POST /delivery/templates/fetch` | Browse OSDU's schemas for the Templates page: search them, or fetch one kind's schema bundled with every schema it refers to, through a delivery flow's OSDU connection. Each queues a compute task (`delivery-schema-search`, `delivery-schema-fetch`) that a node runs with the flow's credentials; nothing is saved |
