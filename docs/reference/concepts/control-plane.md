...
| `GET /schedules` (`repoId`, `pipelineId`, `source`, `enabled`, `search`), `GET /schedules/{id}`, `GET /schedules/{id}/definition`, `GET /schedules/{id}/plan` | Schedules, the YAML behind a git-declared one, and the members a fire runs in wave order |
...
| `GET /delivery/...` | The delivery ledger reads: a flow's stats, records, and submissions; one record with its attempts and activities; one submission with its attempts; the audit trail; the mappings and snapshots the repositories hold. Documented in [Delivery operations](../../delivery/operations.md) |
...
| `POST /delivery/submissions`, `POST /delivery/flows/{pipelineId}/release`, `POST /delivery/flows/{pipelineId}/probe`, `POST /delivery/records/{key}/release`, `/redeliver`, `/verify`, `/read`, `/delete` | The delivery interventions: a manifest notification queues the deliver run; release, redeliver, and verify act on the ledger under the caller's name or queue a run; probe, read-back, and delete queue a compute task for a node. `POST /delivery/ledger/prune` additionally requires `admin`. Documented in [Delivery operations](../../delivery/operations.md) |

Author group (any authenticated user): `POST /repos/sources/{id}/proposals` (src/SqlFlow.ControlPlane/Api/FlowProposalEndpoints.cs), described under managed sync below.
...
3. Runs the exact same `CatalogSync` as the CLI's `sqlflow db sync`, passing the source's flow selection (see below): pipelines are upserted by identity, the mapping and snapshot documents next to the flows are recorded, the git-declared schedules are mirrored, and run artifacts under the tree are imported.
...
The delivery API's `POST /api/v1/delivery/submissions` (the manifest notification) is the same trigger with `operation: deliver`, the submission's `drop`, `parameters`, and `force`, and answers 202 with the run id; see [Delivery operations](../../delivery/operations.md).
