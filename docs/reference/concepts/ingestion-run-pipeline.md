...
1. **Feature guard**: `versioning.temporalHistory` and `versioning.insertUnknownDimensionRow` are rejected up front with explicit "not yet implemented" errors instead of being silently ignored.
...
11. **Optional truncate**: `target.truncateBeforeLoad` issues `TRUNCATE TABLE` on the target.
