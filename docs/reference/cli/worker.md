...
1. **The catalog's YAML snapshot** is probed first. It can only serve a document that executes from a bare single-file copy, and a delivery flow never does: its mappings and snapshots live beside it in the repository tree (`DeliveryFlowDocument.RequiresRepoTree` is always true), so for OSDU Delivery this step always hands over to git.
