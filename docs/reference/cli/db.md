...
`deactivated` counts flows a selection-scoped managed sync excluded; the CLI never passes a selection, so it is always 0 here, and a flow that left the folder counts under `removed`. `documents` tallies mappings, the types the retrieval flows cache, and snapshots together. At most the first 20 warnings print to stderr as `WARN  <text>`; warnings do not change the exit code.
...
Project the sample estate (samples/recall-welllog: two flows, the delivery flow and the retrieval flow that keeps its cache of four types, two mappings and one reference snapshot version) into the catalog under the repository name `recall-welllog`, from a fresh clone with no local run history:

```bash
sqlflow db sync samples/recall-welllog --repo recall-welllog
# OK   synced 'samples/recall-welllog': pipelines +2 added, 0 updated, 0 unchanged, 0 deactivated, 0 removed; runs +0 added (0 events), 0 known, 0 unreadable; documents +7 added, 0 updated, 0 removed, 0 invalid.
```
...
- [The ledger](../../delivery/ledger.md): the `delivery` schema, `delivery.Mapping`, `delivery.Snapshot` and `delivery.Template`, retention, provisioning.
