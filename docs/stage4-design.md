...
- **One database, and a login per tier; two where an estate must.**
  - The shipped shape: no connection of its own, so the `osdu` schema lives in the catalog database, and work that commits with the catalog (the repository sync's mapping, cache and interface rows) joins the catalog's transaction.
  - Given a connection of its own, it lives in a database of its own, which is the only way to separate the two on Azure SQL (no cross-database statement, and often another server). The repository sync then commits its own work on the module's connection and reconciles idempotently from the repository, so a sync that fails after that commit is settled by the next one.
