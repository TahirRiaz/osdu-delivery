...
| `SQLFLOW_DROPOFF_ROOT` | control plane, compute nodes | Where files uploaded through the drop-off API land (`POST /api/v1/delivery/dropoffs`), for a submission to point at afterwards ([submitting-records.md](delivery/submitting-records.md)). The control plane writes there and every node reads there, so both need it and both need it set to the same location: a container the control plane's identity can write and each node's identity can read. It is one name on both sides on purpose, because a submission may point inside it and the node re-checks that before it delivers; two settings could disagree and refuse every upload. Unset, the API offers no drop-off area at all. A file too large to send through the control plane is written straight to storage under a URL the control plane hands out, which needs this area to be on Azure Storage and the control plane's identity to hold **Storage Blob Delegator** on the account in addition to the role that lets it write; without either, `GET /api/v1/delivery/dropoff-area` answers `signedUploads: false` and every upload goes through the control plane. |
