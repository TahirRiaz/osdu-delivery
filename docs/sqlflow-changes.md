...
| `04fa03f` | The long-lived clone that serves a repository's tree, log and window comparisons was fetched but never moved. A fetch updates only the remote-tracking branch, and every reader of that clone reads HEAD, so after the first clone the repo page's content listing and the git history stayed at the commit the clone was made from however far the branch moved, and a remote whose history was rewritten was never followed. | A fetch that brings new commits moves HEAD to the fetched tip of the tracked branch (a hard reset of the read-only clone), so the tree, the log and a comparison read the branch as it is. Two tests in `GitHistoryTests` fail without it. |
| `de5fdc4` | The sync loaded an existing repo row with `FindAsync`, which follows the context's default tracking. The control plane pools its catalog context as `NoTracking`, so every managed sync of a repo that already had a row changed an untracked copy: the pipelines moved to paths relative to the new clone while the repo kept the root, remote and sync time of whichever sync created it. Anything combining the two (a node locating a flow file for a queued operation) looked in a folder that did not hold it. | The repo row is loaded `AsTracking`, as the pipelines already were. `Sync_UnderNoTracking_MovesTheRepoRoot_WhenTheEstateIsSyncedFromAnotherFolder` fails without it. |
...
| `44e8fa2` | The workbench's `SummaryStrip` takes the narrowest a cell may be (`minCellWidth`, default the 200px it always used), so a page with more headline facts than five keeps them on one line instead of stranding the last on a line of its own. Generic: any page that reads a row against six or seven facts wants it. |
| `14d9b0c` | The workbench's `IdChip` renders its `display` as any element rather than a string, and its value span or link carries the full id as `data-value`. A GUID's first eight characters suited the string form; an id whose recognisable part is not its start (a name, a typed id whose tail tells records apart) can now be shown clipped with a hover panel of its own, and a test or a script reads the whole id from the data attribute rather than from what is rendered. Generic: any module whose ids are not GUIDs wants it. |
