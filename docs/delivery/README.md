...
pages describe the one flow kind that runs on it, `flowType: delivery`, and the ledger behind it.

| Page | What it covers |
| --- | --- |
| [design.md](design.md) | The design: why the drop is a storage handoff, the four render inputs and the render context, identity, change detection, the ledger, the delivery protocols, the document model, the preflight gate, streaming. Section numbers are referenced from the code. |
| [documents.md](documents.md) | The flow and mapping documents key by key. |
...
| [protocols.md](protocols.md) | The named delivery protocols (`osduRecord`, `osduWellLog`) and how to add one. |
...
| Path | Responsibility |
| --- | --- |
| `src/SqlFlow.Delivery` | The whole domain: model, identity, canonical JSON and hashing, the document loader and the flow kind, snapshots, rendering, planning, the preflight gate, the HTTP runtime, storage (drop reader, snapshot store, writers), the ledger over the catalog, the engine (intake, worker, verifier, known state, protocols, snapshot capture), the run executor, the compute operations, and the sync of mappings and snapshots into the catalog. |
...
| `gui/src/features/delivery` | The delivery overview, the flow tabs (stats, records, submissions), the record page, the submission page, the audit trail, mappings and snapshots. |
